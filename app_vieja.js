...
var humanButtonMessage = require('./SQLtoJSONmodule.js').humanButtonMessage;
var humanPostBackMessage = require('./SQLtoJSONmodule.js').humanPostBackMessage;
...
function callbackFunction(err, rows, fields) {
  if (!err) {
    ...
    allRows = SQLtoJSONmodule(rows, jsonObjet);
  } else {
...
function receivedMessage(event) {
  ...
  if (messageText) {
    sendTextMessage(senderID, allRows);
...
function sendTextMessage(recipientId, messageText) {
  for (var i = 0; i < messageText.length; i++) {
   /* console.log(humanButtonMessage(messageText[i]));*/
    var messageData = {
      recipient: {
        id: recipientId
      },
      message: {
        attachment: humanButtonMessage(messageText[i])
      }
    };
